import numpy as np
import tqdm
from losses.dsm import anneal_dsm_score_estimation, ncsnv2_dsm_score_estimation
...
__all__ = ['AnnealRunner_v2_feature']


class AnnealRunner_v2_feature():
    ...
    def train(self):
        ...
        if self.config.data.dataset == 'CIFAR100':
            dataset = CIFAR100(os.path.join(self.args.run, 'datasets', 'cifar100'), train=True, download=True,
                              transform=tran_transform)
            test_dataset = CIFAR100(os.path.join(self.args.run, 'datasets', 'cifar100_test'), train=False, download=True,
                                   transform=test_transform)
        ...
        elif self.config.data.dataset == 'TinyImageNet':
            dataset = TinyImageNet(os.path.join(self.args.run, 'datasets', 'tiny-imagenet-200', 'train'), train=True)
            test_dataset = TinyImageNet(os.path.join(self.args.run, 'datasets', 'tiny-imagenet-200', 'val'), train=False)

        elif self.config.data.dataset == 'CIFAR10_feature':
            latent_all = np.load(os.path.join('latent_all.npy'))
            labels_all = np.load(os.path.join('label_all.npy'))
            dataset = TensorDataset(torch.Tensor(latent_all), torch.Tensor(labels_all))
            test_dataset = TensorDataset(torch.Tensor(latent_all), torch.Tensor(labels_all))
        ...
        score = FeatureNet(self.config).to(self.config.device)
        score = torch.nn.DataParallel(score)
        optimizer = self.get_optimizer(score.parameters())
        ...
        for epoch in range(self.config.training.n_epochs):
            ...
            for i, (X, y) in enumerate(dataloader):
                ...
                if self.config.training.algo == 'dsm':
                    ...
                    loss = ncsnv2_dsm_score_estimation(score, X, labels, sigmas, self.config.training.anneal_power)
