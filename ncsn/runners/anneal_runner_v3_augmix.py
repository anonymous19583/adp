...
__all__ = ['AnnealRunner_v3_augmix']
...
class AnnealRunner_v3_augmix():
    ...
    def augment_and_mix_tensor(self, x, th_rate, normalize=False):
        x = x.to('cpu').numpy()
        x_mix = x.copy()
        for i in range(x.shape[0]):
            x[i], x_mix[i] = augment_and_mix(x[i], th_rate, normalize=normalize)
        x = torch.Tensor(x).to('cuda')
        x_mix = torch.Tensor(x_mix).to('cuda')
        return x, x_mix
    ...
    def train(self):
        ...
        for epoch in range(self.config.training.n_epochs):
            for i, (X, y) in enumerate(dataloader):
                ...
                X_aug, X_mix = self.augment_and_mix_tensor(X, th_rate=self.config.training.th_rate, normalize=False)
                step += 1
                if self.config.training.algo == 'dsm':
                    loss = ncsnv2adv_dsm_score_estimation(score, X_mix, X_aug, labels, sigmas, self.config.training.anneal_power)
                ...
                if step % 100 == 0: # Estimation via data with Gaussian noise
                    score.eval()
                    try:
