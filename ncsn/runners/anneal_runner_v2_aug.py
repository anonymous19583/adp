    ...
    def train(self):
        ...
        for epoch in range(self.config.training.n_epochs):
            for i, (X, y) in enumerate(dataloader):
                ...
                X_aug = self.augment_and_mix_tensor(X, normalize=False)
                step += 1
                if self.config.training.algo == 'dsm':
                    loss = ncsnv2adv_dsm_score_estimation(score, X, X_aug, labels, sigmas, self.config.training.anneal_power)
