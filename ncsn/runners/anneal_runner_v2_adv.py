...
class AnnealRunner_v2_adv():
    ...
    def train(self):
        ...
        elif self.config.data.dataset == 'FEATURE':
            dataset = cifar10_feature(os.path.join(self.args.run, 'datasets', 'feature'), split='train')
            test_dataset = cifar10_feature(os.path.join(self.args.run, 'datasets', 'cifar10_feature'), split='test')
        ...
        # Get pre-trained classification network
        clf = eval(self.config.clf.network)().to('cuda')
        clf = torch.nn.DataParallel(clf)
        states_att = torch.load(os.path.join(path_root, 'adv_training/run/logs', self.config.clf.name, 'checkpoint.pth'), map_location='cuda')
        opt_clf = optim.Adam(clf.parameters(), lr=0.001, weight_decay=0, betas=(0.9,0.999), amsgrad=False)
        clf.load_state_dict(states_att[0])
        ...
        for epoch in range(self.config.training.n_epochs):
            for i, (X, y) in enumerate(dataloader):
                ...
                # Get pgd attacked data
                pgd_iters = 40
                pgd_eps = self.config.training.pgd_eps
                X_pgd_list = self.pgd(X, X, y, clf, opt_clf, ptb=pgd_eps, iters=pgd_iters)
                for j in range(pgd_iters):
                    step += 1
                    if self.config.training.algo == 'dsm':
                        #loss = anneal_dsm_score_estimation(score, X, labels, sigmas, self.config.training.anneal_power)
                        loss = ncsnv2adv_dsm_score_estimation(score, X, X_pgd_list[j], labels, sigmas, self.config.training.anneal_power)
                    elif self.config.training.algo == 'ssm':
                        loss = anneal_sliced_score_estimation_vr(score, X_pgd_list[j], labels, sigmas,
