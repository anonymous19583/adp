...
class fgsm():
    ...
    def fgsm(self, x, xgrad):
        # By random start, start from xprime
        if self.args.norm==-1:
            return torch.clamp(x + self.args.ptb/256.*xgrad.sign(), min=0.0, max=1.0)
    ...
    def train(self): # Train by adversarial training
        ...
        for epoch in range(self.config.training.n_epochs):
            ...
            for i, (x,y) in enumerate(dataLoader):
                ...
                loss.backward()
                xgrad = xprime.grad.data
                x_fgsm = self.fgsm(x, xgrad)
            ...
            for i, (x,y) in enumerate(dataLoader):
                ...
                # Perturbed data
                xprime.requires_grad_(True)
                yhat_b = network(xprime)
                optimizer.zero_grad()
                loss_b = criterion(yhat_b, y)
                loss_b.backward()
                xgrad = xprime.grad.data
                x_fgsm = self.fgsm(x, xgrad)
                yhat_a = network(x_fgsm)
            ...
            for i, (x,y) in enumerate(testLoader):
                ...
                loss = criterion(yhat, y)
                loss.backward()
                xgrad = xprime.grad.data.detach().requires_grad_(False)
                x_fgsm = self.fgsm(x, xgrad)
                yhat = network(x_fgsm)
    ...
    def test(self): # Test for adversarially attacked images
        ...
        for i, (x,y) in enumerate(testLoader):
            ...
            if i==0:
                ...
                ax_adv.axis("off")
                view_xadv = x_fgsm.view(grid_size**2, x_fgsm.size(1), x_fgsm.size(2), x_fgsm.size(3))
                if x_fgsm.size(1)==1:
